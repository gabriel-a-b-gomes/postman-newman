...
module.exports = function (options, callback) {
    ...
    getOptions(options, function (err, options) {
        ...
        if (sslClientCertList) {
            sslClientCertList.forEach((ssl) => {
                if (ssl.key.src.startsWith('~')) {
                    ssl.key.src = expandHome(ssl.key.src);
                }
                if (ssl.cert.src.startsWith('~')) {
                    ssl.cert.src = expandHome(ssl.cert.src);
                }
            });
        }
